...
impl Image {
    ...
    pub fn try_into_dynamic(self) -> Result<DynamicImage, IntoDynamicImageError> {
        ...
        match self.texture_descriptor.format {
            TextureFormat::R8Unorm => {
                ImageBuffer::from_raw(width, height, data.into_owned()).map(DynamicImage::ImageLuma8)
            }
            TextureFormat::Rg8Unorm => {
                ImageBuffer::from_raw(width, height, data.into_owned()).map(DynamicImage::ImageLumaA8)
            }
            TextureFormat::Rgba8UnormSrgb => {
                ImageBuffer::from_raw(width, height, data.into_owned()).map(DynamicImage::ImageRgba8)
            }
